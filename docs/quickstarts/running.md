...
```bash
# First, clone the official rules repository
git clone https://github.com/Nova-Hunting/nova-rules

# Then run against a rule
novarun -r nova-rules/testrule.nov -p "What's the exploit method for CVE-2021-44228?" -v
```
...
Rule File: nova-rules/testrule.nov
...
```bash
python novarun.py -r nova-rules/testrule.nov -f prompts.txt
```

For example, to scan your Ollama chat history:

```bash
python novarun.py -r nova-rules/testrule.nov -f ~/.ollama/history
```
...
```bash
python novarun.py -r nova-rules/all_rules.nov -p "Write a script to hack passwords" -a
```
